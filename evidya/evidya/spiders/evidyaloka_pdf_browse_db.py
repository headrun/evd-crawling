...
class EvdmetaBrowsepdf3(scrapy.Spider):
    name = 'evidyaloka_pdf_browse3'
    ...
    def __init__(self,state = '', district = '',  **kwargs):
        super(EvdmetaBrowsepdf3,self).__init__(**kwargs)
        ...
        get_query_param = "select sk, state_name, district_name from evidya_crawl where crawl_status=0 order by rand() limit 1"
        self.cur.execute(get_query_param)
        profiles_list = [i for i in self.cur.fetchall()]
        if not profiles_list:
            self.all_schools = []
        if profiles_list:
                 self.counter = 0
                 self.final_json = OrderedDict()
                 self.sk, self.state, self.district = profiles_list[0]
                 print self.sk, self.state, self.district
                 self.cur.execute("update evidya_crawl set crawl_status=9 where sk='%s'"% self.sk)
                 self.file_name = "%s%s%s" % (self.state.replace(' ', '_'), self.district.replace(' ', '_'),'.csv')
                 with open('total_page_stats.csv') as csvdata:
                        ...
                        self.all_schools = [ons[9] for ons in self.one_state_data if ons[9]]
                        print self.all_schools
                        print len(self.all_schools)
                        #print len([line for line in self.csv_data if line[-1]== "Maharashtra"])
                        #print len([line for line in self.csv_data if line[-1]== "Madhya Pradesh"])
                        #print len([line for line in self.csv_data if line[-1]== "Uttar Pradesh"])
                        #print len([line for line in self.csv_data if line[-1]== "Karnataka"])
    ...
    def spider_closed(self, spider):
        with open("%s%s" % ('Complete_csv_json/', self.file_name.replace('.csv', '.json')), 'w+') as fin:
            json.dump(self.final_json, fin)
        #self.cur.execute("update evidya_crawl set crawl_status=1 where sk='%s'"% self.sk)

    def is_path_file_name(self, excel_file_name):
        #if os.path.isfile(excel_file_name):
                #os.system('rm %s' % excel_file_name)
        oupf = open("%s%s" % ('Complete_csv_json/', excel_file_name), 'w+')
